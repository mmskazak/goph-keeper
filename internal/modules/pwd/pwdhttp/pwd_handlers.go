...
func NewPwdHandlersHTTP(service pwdservices.IPwdService, sugaredLogger *zap.SugaredLogger) PwdHandlers {
	return PwdHandlers{
		pwdService: service,
...
func (p PwdHandlers) SavePassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		p.zapLogger.Errorf("Error make SavePwdDTOFromHTTP: %v", err)
		http.Error(w, "", http.StatusBadRequest)
		return
	}
	err = p.pwdService.SavePassword(r.Context(), &savePwdDTO)
	if err != nil {
		p.zapLogger.Errorf("Error work pwdService SavePwdDTO: %v", err)
	...
	if err != nil {
		p.zapLogger.Errorf("Error writing response for handler SavePassword: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
}
...
func (p PwdHandlers) GetPassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		p.zapLogger.Errorf("Error make GetPwdDTOFromHTTP: %v", err)
		http.Error(w, "", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			p.zapLogger.Infoln("Get password not found")
			http.Error(w, "", http.StatusNotFound)
			return
		}
		p.zapLogger.Errorf("Error work pwdService GetPwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		p.zapLogger.Errorf("Error writing response for handler GetPwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
}
...
func (p PwdHandlers) DeletePassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		p.zapLogger.Errorf("Error make DeletePwdDTOFromHTTP: %v", err)
		http.Error(w, "", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			p.zapLogger.Infoln("Deleted password not found")
			http.Error(w, "", http.StatusNotFound)
			return
		}
		p.zapLogger.Errorf("Error work pwdService DeletePwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
	...
	_, err = w.Write([]byte("OK"))
	if err != nil {
		p.zapLogger.Errorf("Error writing response for handler DeletePwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
}
...
func (p PwdHandlers) GetAllPasswords(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		p.zapLogger.Errorf("Error make AllPwdDTOFromHTTP: %v", err)
		http.Error(w, "", http.StatusBadRequest)
		return
	}
	allPasswords, err := p.pwdService.GetAllPasswords(r.Context(), &allPwdDTO)
	if err != nil {
		p.zapLogger.Errorf("Error work pwdService GetAllPwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		p.zapLogger.Errorf("Error writing response for handler GetAllPwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
}
...
func (p PwdHandlers) UpdatePassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		p.zapLogger.Errorf("Error make UpdatePwdDTOFromHTTP: %v", err)
		http.Error(w, "", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		if errors.Is(err, ErrUpdatedRecordNotFound) {
			p.zapLogger.Errorf("Error work pwdService UpdatePwdDTO: %v", err)
	...
	_, err = w.Write([]byte("OK"))
	if err != nil {
		p.zapLogger.Errorf("Error writing response for handler UpdatePwdDTO: %v", err)
		http.Error(w, "", http.StatusInternalServerError)
		return
	}
}
